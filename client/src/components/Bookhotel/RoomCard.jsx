...
export const RoomCard = ({ data, i }) => {
    ...
    const { data: hotelData, pictures } = useSelector(store => store.singleHotel);
    const arrowStyles = {
        cursor: "pointer",
        pos: "absolute",
        top: "50%",
        ...
        <Box border='1px solid rgba(0,0,0,0.4)' bg='white' h='100%'>
            ...
            >
                <Flex w="full" overflow="hidden" pos="relative">
                    <Flex w="full" h={{ base: "60vw", sm: "30vw", lg: '200px' }} {...carouselStyle}>
            ...
            <Box p='10px'>
                <Heading fontSize='16px' minH='40px'>{data.text}</Heading>
            ...
            <Box p='10px' borderTop={'1px solid rgba(0,0,0,0.4)'} ref={containerRef}>
                ...
                <RadioGroup onChange={setValue} value={value} display='flex' flexDir={'column'} mt='10px'>
                ...
                <Heading fontSize={'22px'} mt='20px'>
                    ${pictures.length && (Number(hotelData['uitk-text 8'].substring(1)) + Number(value))}
                </Heading>
                <Text fontSize='15px'>
                    ${pictures.length && (Number(hotelData['uitk-text 8'].substring(1)) + data.value + Number(value))} total
                </Text>
                <Portal containerRef={containerRef}>
                    <Flex justifyContent={'space-between'} alignItems='center'>
                        <Flex alignItems='center' gap='1px' fontSize='14px' color='teal' mt='6px'
                            cursor='pointer' wrap='nowrap'>
                            <Text whiteSpace={'nowrap'}>
                                Price details
                            </Text>
                            <Icon as={RiArrowRightSLine} fontSize='15px' />
                        </Flex>
                        {!data.sold ? <Button colorScheme='teal' h='35px'>
                            Reserve
                        </Button> : (
